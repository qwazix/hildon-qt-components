    ...
    : QAbstractListModel(parent)
{
    ...
    roles[FolderRole] = "fileIsDir";
    setRoleNames(roles);

    d = new QDeclarativeFolderListModelPrivate;
    d->model.setFilter(QDir::AllDirs | QDir::Hidden | QDir::Files | QDir::Drives | QDir::NoDotAndDotDot);
...
void QDeclarativeFolderListModel::setShowFiles(bool on)
{
    if (!(d->model.filter() & QDir::Files) == !on)
        return;
    if (on)
        d->model.setFilter(d->model.filter() | QDir::Files | QDir::AllDirs | QDir::Hidden | QDir::Drives);
    else
...
bool QDeclarativeFolderListModel::newFolder(QString name) {
    QString path = d->folder.toLocalFile();
    ...
    if (success) {
        setFolder(QUrl::fromLocalFile(path));
    }
